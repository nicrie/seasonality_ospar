...
# OSPAR Beach Litter surveys; needed to get relevant countries in the North-East Atlantic (NEA) region
# -----------------------------------------------------------------------------
ospar = open_datatree("data/beach_litter/ospar/preprocessed.zarr", engine="zarr")
countries_nea = np.unique(ospar["preprocessed/absolute"].country)

# EUMOFA Exports from fishing products (fisheries + aquaculture)
# -----------------------------------------------------------------------------
path = "data/economic/exports/eumofa/"
# Get all .csv files in the directory
files = [f for f in os.listdir(path) if f.endswith(".csv")]
# Load all files into a single DataFrame using dask
# Imports + Exports within/outside EU
trade_EU = dd.read_csv(path + "*_EU_*.csv", sep=";")
trade_non_EU = dd.read_csv(path + "*_non-EU_*.csv", sep=";")

# Some renaming of columns is necessary for consistency
...
# Select exports from NEA countries
exports_nea = exports[exports["country"].isin(countries_nea)]

# Compute total exports per year, month, country, commodity group and species
